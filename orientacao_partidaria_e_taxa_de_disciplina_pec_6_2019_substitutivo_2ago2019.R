...
orientacao_partidos_n <- orientacao_partidos %>%
  ...
  mutate(partido = str_replace_all(partido, "REDE", "Rede")) %>%
  arrange(id)
...
joined_data <- votos_deputados_n %>%
  ...
  filter(id == "CD190208") # código se refere à votação do texto-base
# ACIMA, caso opte por outra votação, apenas trocar o código da votação
...
write.csv(joined_data_2, "partido_orientacao_voto_pec_6_2019_substitutivo.csv")
