...
df_total_4 <- df_total %>%
  ...
  mutate(comissao_provisoria_perc = ifelse(tipo_orgao == "Órgão provisório", int_perc, NA))
...
df_total_plot_perc <- ggplot(df_total_4, aes(x = nome_partido, y = int_perc, fill = tipo_orgao, width = 0.8)) +
  ...
  #         inherit.aes = TRUE) +
  coord_flip() + 
  scale_fill_manual("tipo_orgao", values = c("Comissão executiva" = "#666666", "Comissão interventora" = "#7E0F08", 
                                             "Órgão definitivo" = "#DADADA", "Órgão provisório" = "#A80000"),
                    guide = guide_legend(reverse = TRUE)) +
...
ggsave("test.tiff", units="in", dpi=900)
